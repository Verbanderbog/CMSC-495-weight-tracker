...
public class Graph {
  ...
  void startDateChange() {
    try {
      long startDate = startDatePicker.getValue().toEpochDay();
      long endDate = endDatePicker.getValue().toEpochDay();
      if (startDate < endDate) {
        dateAxis.setLowerBound(startDate);
      } else {
        startDatePicker.setValue(LocalDate.ofEpochDay(endDate-1));
  ...
  void endDateChange() {
    try {
      ...
      long endDate = endDatePicker.getValue().toEpochDay();
      if (startDate < endDate) {
        dateAxis.setUpperBound(endDate);
      } else {
        endDatePicker.setValue(LocalDate.ofEpochDay(startDate+1));
